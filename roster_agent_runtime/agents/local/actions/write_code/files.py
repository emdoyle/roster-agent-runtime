...
from roster_agent_runtime.agents.local.parsers.plan import ImplementationPlanAction
...
def execute_implementation_plan(
    agent: LocalAgent, implementation_plan: list[ImplementationPlanAction]
):
    implementation_actions = ImplementationPlanParser.parse(implementation_plan)

    modified_filenames = [
        action.filename for action in implementation_actions if action.type == "modify"
    ]
